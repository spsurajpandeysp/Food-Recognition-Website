...
@app.route('/flaskapi/findname', methods=['POST'])
def get_data():
    ...
    model.load_state_dict(torch.load('foodresnet50.h5', map_location=device))
    model.to(device)
    model.eval()
    ...
    url = f"http://localhost:5000/api/fooddata/{food_name}"
    print(url)
